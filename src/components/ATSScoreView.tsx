...
export const ATSScoreView: React.FC<ATSScoreViewProps> = ({
    ...
    return (
      <div className="max-w-xl w-full mx-auto animate-slide-up text-center py-20">
        <div className="glass-panel p-10 flex flex-col items-center space-y-6 relative overflow-hidden">
          ...
          <div className="w-16 h-16 rounded-full border-4 border-slate-850 border-t-indigo-500 animate-spin relative z-10" />
          <div className="relative z-10 space-y-2">
            <h2 className="text-xl font-bold text-white tracking-wide">Recruiter Screening Sandbox</h2>
  ...
  if (error || !scoreData) {
    return (
      <div className="max-w-xl w-full mx-auto text-center py-20 animate-slide-up">
        <div className="glass-panel p-8 space-y-6 border-red-500/20 bg-red-950/5">
          <div className="w-14 h-14 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-400 mx-auto shadow-lg shadow-red-500/5">
            <AlertCircle size={26} />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white tracking-wide">ATS Evaluation Pipeline Failed</h2>
            <p className="text-sm text-slate-400 mt-2 leading-relaxed">{error || 'Unknown parsing issue occurred.'}</p>
          </div>
  ...
  const getScoreColorClass = (val: number) => {
    if (val >= 80) return 'text-emerald-400 stroke-emerald-500/80 drop-shadow-[0_0_8px_rgba(16,185,129,0.3)]';
    if (val >= 60) return 'text-amber-400 stroke-amber-500/80 drop-shadow-[0_0_8px_rgba(245,158,11,0.3)]';
    return 'text-red-400 stroke-red-500/80 drop-shadow-[0_0_8px_rgba(239,68,68,0.3)]';
  };

  const getScoreBgClass = (val: number) => {
    if (val >= 80) return 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400';
    if (val >= 60) return 'bg-amber-500/10 border-amber-500/20 text-amber-400';
  ...
  return (
    <div className="max-w-4xl w-full mx-auto space-y-6 animate-slide-up relative">
      ...
      <div className="glass-panel p-6 md:p-8 flex flex-col lg:flex-row items-center justify-between gap-8 relative overflow-hidden border-slate-800/80">
        <div className="flex-1 space-y-4 text-center lg:text-left">
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2.5">
            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-full flex items-center gap-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-slate-400 max-w-lg leading-relaxed">
              Evaluating your document compatibility for the <strong className="text-slate-200">{role}</strong> opening at <strong className="text-slate-200">{company}</strong>.
            </p>
          </div>
        </div>

        {/* Animated Radial Score Gauge Card */}
        <div className="bg-slate-950/40 border border-slate-905 p-6 rounded-2xl flex flex-col md:flex-row items-center gap-6 flex-shrink-0 relative">
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute text-center">
              ...
              <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Match</p>
            </div>
          </div>
          ...
          <div className="w-48 space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold text-slate-400">
                <span className="flex items-center gap-1"><Database size={10} className="text-indigo-400" /> Keyword Density</span>
                <span className="text-slate-200">{keywordMatch}%</span>
              </div>
              <div className="bg-slate-900 rounded-full h-1 overflow-hidden border border-slate-900">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold text-slate-400">
                <span className="flex items-center gap-1"><Layout size={10} className="text-emerald-400" /> Layout &amp; Structure</span>
                <span className="text-slate-200">{formattingScore}%</span>
              </div>
              <div className="bg-slate-900 rounded-full h-1 overflow-hidden border border-slate-900">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold text-slate-400">
                <span className="flex items-center gap-1"><Award size={10} className="text-amber-400" /> Impact &amp; Metrics</span>
                <span className="text-slate-200">{impactScore}%</span>
              </div>
              <div className="bg-slate-900 rounded-full h-1 overflow-hidden border border-slate-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 space-y-4 hover:border-slate-800 transition-all duration-300">
          <div className="flex items-center justify-between border-b border-slate-900 pb-3">
            ...
            <span className="text-[10px] font-extrabold px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-md">
          ...
          {matching_keywords.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {matching_keywords.map((kw: string, i: number) => (
                <span 
                  key={i} 
                  className="bg-slate-950/80 border border-slate-900 hover:border-emerald-500/30 text-slate-300 hover:text-emerald-400 text-xs px-3 py-1.5 rounded-xl flex items-center gap-2 transition-all duration-200 cursor-default group"
                >
                  <div className="w-4 h-4 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                    <Check size={10} />
                  </div>
                  {kw}
                </span>
            ...
            <p className="text-xs text-slate-500 italic py-2">No matching keywords parsed from your resume text.</p>
          )}
        </div>
        ...
        <div className="glass-panel p-6 space-y-4 hover:border-slate-800 transition-all duration-300">
          <div className="flex items-center justify-between border-b border-slate-900 pb-3">
            ...
            <span className="text-[10px] font-extrabold px-2 py-0.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-md">
          ...
          {missing_keywords.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {missing_keywords.map((kw: string, i: number) => (
                <span 
                  key={i} 
                  className="bg-slate-950/80 border border-slate-900 hover:border-amber-500/30 text-slate-300 hover:text-amber-400 text-xs px-3 py-1.5 rounded-xl flex items-center gap-2 transition-all duration-200 cursor-default group"
                >
                  <div className="w-4 h-4 rounded-full bg-amber-500/10 text-amber-400 flex items-center justify-center flex-shrink-0 group-hover:bg-amber-500 group-hover:text-white transition-all">
            ...
            <p className="text-xs text-slate-500 italic py-2">Excellent! Your resume matches all core keywords expected by standard filters.</p>
          )}
        </div>
      </div>

      {/* Constructive Critique Dashboard Card */}
      <div className="glass-panel p-6 md:p-8 space-y-5 relative overflow-hidden">
        {/* Glow effect */}
        ...
        <div className="border-b border-slate-900 pb-3 flex items-center justify-between">
          <h3 className="text-md font-extrabold text-white flex items-center gap-2">
            <ShieldAlert size={18} className="text-brand-500" /> Resume Optimisation Advice
          </h3>
        ...
        {feedback && feedback.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {feedback.map((item: string, idx: number) => {
              ...
              return (
                <div key={idx} className="bg-slate-950/40 border border-slate-900/60 p-4 rounded-xl space-y-2 hover:bg-slate-950/80 transition-all duration-300 flex flex-col justify-between">
                  <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">No revisions recommended. Your resume aligns well with standard structure.</p>
        )}
      </div>
